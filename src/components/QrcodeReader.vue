<template lang="html">
  <div>
    ...
    <div id="scanResult"><div class="container">{{scanResult}}</div></div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    onDecode (content) {
      store.dispatch('addQrCode', [this.$session.get('uid'), content])
      this.$emit('decode', content)
      this.paused = true
      this.scanResult = 'QR код успешно отсканирован. Данные по нему будут доступны в ближайшее время!'
    },
    onLocate (location) {
